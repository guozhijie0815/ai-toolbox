...
- **工具主导 + 能力卡片**：左栏选工具，中栏按工具展示能力（技能管理 / 配置编辑 / 配置同步 / 模型同步）
- **OpenCode 模型同步**：接入 Modelverse 列表，支持穿梭、排序、改名，写回 `provider.ucloud.models`（自动备份）
- **Claude 配置同步**：仍作为 Claude 专属能力卡片，不再挤在通用 Tab 里
